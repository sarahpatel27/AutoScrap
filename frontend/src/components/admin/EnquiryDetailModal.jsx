...
export default function EnquiryDetailModal({ enquiry, onClose, onUpdateStatus, onDelete }) {
  ...
  const itemCity = enquiry.city || getCityFromPostcode(enquiry.postcode || enquiry.customer?.collectionPostcode, enquiry.customer?.collectionAddress);
  const hasBankDetails = enquiry.bank?.accountNumber || enquiry.bank?.sortCode;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-950/60 p-4 backdrop-blur-xs">
      <div className="relative w-full max-w-3xl overflow-hidden rounded-3xl bg-white shadow-2xl transition-all">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 bg-[#0b2e21] px-6 py-5 text-white">
          ...
          <button
            onClick={onClose}
            className="grid h-9 w-9 place-items-center rounded-xl bg-white/10 text-white transition hover:bg-white/20 cursor-pointer"
            type="button"
          >
            ✕
          </button>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 text-xs border-t border-gray-200 pt-3">
                ...
                <div><span className="text-gray-500">City / Postcode:</span> <strong className="text-[#0f7b4f]">📍 {itemCity} ({enquiry.postcode || 'N/A'})</strong></div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5">
              ...
              <div className="mt-2 space-y-1.5 text-xs text-gray-700">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-gray-900">{enquiry.customer?.collectionAddress || enquiry.postcode}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-extrabold uppercase tracking-wider text-gray-500">
                Bank Payout Details (UK Bank Transfer)
              </span>
              {hasBankDetails ? (
                <span className="rounded-md bg-emerald-100 px-2.5 py-0.5 text-[11px] font-black text-emerald-800 border border-emerald-300">
                  💳 Bank Transfer Registered
                </span>
              ) : (
                <span className="rounded-md bg-amber-100 px-2.5 py-0.5 text-[11px] font-black text-amber-800 border border-amber-300">
                  ⚠️ Pay on Collection (No Bank Details)
                </span>
              )}
            </div>
            ...
            {hasBankDetails ? (
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3 text-xs">
                <div>
                  ...
                  <div className="font-extrabold text-slate-900">{enquiry.bank?.accountName || 'N/A'}</div>
                </div>
                <div>
                  <span className="text-gray-500">Sort Code:</span>
                  <div className="font-mono font-black text-slate-900">{enquiry.bank?.sortCode || 'N/A'}</div>
                </div>
                <div>
                  <span className="text-gray-500">Account Number:</span>
                  <div className="font-mono font-black text-slate-900">{enquiry.bank?.accountNumber || 'N/A'}</div>
                </div>
                {enquiry.bank?.bankName && (
                  <div className="sm:col-span-3 text-gray-500">
                    Bank Name: <strong className="text-gray-900">{enquiry.bank.bankName}</strong>
                  </div>
                )}
              </div>
            ) : (
              <p className="mt-2 text-xs text-gray-500 italic">
                Customer skipped bank payout entry. The collection driver will confirm payment method upon pickup.
              </p>
            )}
          </div>
        ...
        <div className="flex items-center justify-between border-t border-gray-200 bg-gray-50 px-6 py-4">
          <button
            ...
            className="rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-xs font-bold text-red-700 transition hover:bg-red-100 cursor-pointer"
          >
            🗑️ Delete Enquiry
          </button>
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl border border-gray-300 bg-white px-4 py-2 text-xs font-bold text-gray-700 transition hover:bg-gray-100 cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              onClick={handleSave}
              disabled={saving}
              className="rounded-xl border border-0 bg-[#0f7b4f] px-5 py-2 text-xs font-extrabold text-white shadow-xs transition hover:bg-[#075b3a] cursor-pointer"
            >
